...
截至 2026 年 9 月 13 日，GitHub Releases 中最新公开稳定版仍为 [v1.0.0（2026 年 8 月 6 日）](https://github.com/ASL-Vanity/LinkLake/releases/tag/v1.0.0)。`v1.1.0` 和 `v1.1.1` 标签是用于门禁流程的不可变标签，但没有对应的 GitHub Release，也不应当被当作已发布的可下载版本。`v1.1.2` 是当前发布目标；只有标签工作流完成全部既有门禁并创建 Release 后，相关包和镜像才构成公开发行资产。
...
As of September 13, 2026, the latest public stable GitHub Release remains [v1.0.0 (August 6, 2026)](https://github.com/ASL-Vanity/LinkLake/releases/tag/v1.0.0). The `v1.1.0` and `v1.1.1` tags are immutable tags used by release-gate attempts; they do not have corresponding GitHub Releases and must not be treated as published downloadable versions. `v1.1.2` is the current release target. Its packages and images become public release assets only after the tag workflow passes the existing gates and creates the Release.
