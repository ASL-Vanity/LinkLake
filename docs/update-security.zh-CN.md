...
生产公钥 `linklake-production-2026-08-a` 已签署并通过验证的 `v1.0.0` 更新清单，并继续登记用于 `1.0.x` 发布线。后续每个标签 Release 都会在签名步骤关闭失败，除非 `LINKLAKE_RELEASE_SIGNING_KEY_ID` 选择对该版本有效的已登记生产密钥，且 `LINKLAKE_RELEASE_SIGNING_KEY_B64` 提供与其匹配的私钥种子。禁止使用开发夹具绕过这一安全阻断。
