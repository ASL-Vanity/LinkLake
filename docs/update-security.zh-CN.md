...
生产公钥 `linklake-production-2026-08-a` 已签署并通过验证的 `v1.0.0` 更新清单，目前登记的版本上限为 `1.99.99`。该上限只是紧急兼容边界，并不代表整个 `1.x` 系列都应重复使用同一密钥；正常轮换应在到达上限前提前登记新公钥，并先发布同时信任新旧公钥的更新器。后续每个标签 Release 都会在签名步骤关闭失败，除非 `LINKLAKE_RELEASE_SIGNING_KEY_ID` 选择对该版本有效的已登记生产密钥，且 `LINKLAKE_RELEASE_SIGNING_KEY_B64` 提供与其匹配的私钥种子。禁止使用开发夹具绕过这一安全阻断。
