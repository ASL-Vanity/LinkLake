...
本文对应 LinkLake v1.1 功能线和当前 `v1.1.2` 发布目标的命令与配置。GitHub Releases 中最新公开稳定版仍为 `v1.0.0`；仓库版本号不代表公开发布已经完成。安装时使用与目标平台匹配的完整安装包，并核对包的版本、校验和与签名。涉及路径、域名、端口和凭据的示例均需替换为自己的值。
