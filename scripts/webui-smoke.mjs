...
try {
  ...
  const [createOperatorResponse] = await Promise.all([
    page.waitForResponse(response => response.url().endsWith('/api/v1/users') && response.request().method() === 'POST', { timeout: 60_000 }),
    page.locator('#user-form').evaluate(form => form.requestSubmit()),
  ]);
  assert(createOperatorResponse.ok(), '创建运维用户的界面请求失败');
  ...
  const [updateOperatorResponse] = await Promise.all([
    page.waitForResponse(response => response.url().endsWith('/api/v1/users/smoke_operator') && response.request().method() === 'PUT', { timeout: 60_000 }),
    page.locator('#user-form').evaluate(form => form.requestSubmit()),
  ]);
  assert(updateOperatorResponse.ok(), '编辑运维用户的界面请求失败');
  ...
  const [resetOperatorResponse] = await Promise.all([
    page.waitForResponse(response => response.url().endsWith('/api/v1/users/smoke_operator/reset-password') && response.request().method() === 'POST', { timeout: 60_000 }),
    page.locator('#reset-password-form').evaluate(form => form.requestSubmit()),
  ]);
  assert(resetOperatorResponse.ok(), '重置运维用户密码的界面请求失败');
