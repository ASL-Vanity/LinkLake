...
impl FleetHealthCatalog {
    ...
    pub(crate) fn create_dns_failover(
        ...
        transaction.execute(
            "INSERT INTO fleet_dns_failovers (
                ...
                proxied, enabled, cooldown_seconds, frozen, freeze_reason,
    ...
    pub(crate) fn update_dns_failover(
        ...
        transaction.execute(
            "UPDATE fleet_dns_failovers SET
                ...
                enabled = ?10, cooldown_seconds = ?11, updated_unix_seconds = ?12
             WHERE id = ?1",
            params![
                id.to_string(),
                request.name,
    ...
    pub(crate) fn list_dns_failovers(&self) -> anyhow::Result<Vec<FleetDnsFailover>> {
        let mut statement = self.database.prepare(
            "SELECT id, name, hostname, record_type, zone_id, record_id, token_env,
                    ttl, proxied, enabled, cooldown_seconds, frozen, freeze_reason,
                    current_peer_id, current_target, last_switch_unix_seconds,
                    next_change_not_before_unix_seconds, last_switch_reason,
                    last_error_summary, pending_operation_id, pending_peer_id,
                    pending_target, pending_reason, pending_started_unix_seconds,
    ...
    pub(crate) fn plan_dns_changes(
        ...
        for id in ids {
            ...
            let reason = dns_switch_reason(&transaction, &failover, candidate.peer_id)?;
...
fn read_dns_failover_row(row: &rusqlite::Row<'_>) -> rusqlite::Result<FleetDnsFailover> {
    ...
    Ok(FleetDnsFailover {
        ...
        freeze_reason: row.get(12)?,
        current_peer_id: optional_uuid(row.get(13)?, 13)?,
        current_target: row.get(14)?,
        last_switch_unix_seconds: optional_nonnegative(row.get(15)?),
        next_change_not_before_unix_seconds: nonnegative(row.get(16)?),
        last_switch_reason: row.get(17)?,
        last_error_summary: row.get(18)?,
        pending_operation_id: optional_uuid(row.get(19)?, 19)?,
        pending_peer_id: optional_uuid(row.get(20)?, 20)?,
        pending_target: row.get(21)?,
        pending_reason: row.get(22)?,
        pending_started_unix_seconds: optional_nonnegative(row.get(23)?),
        pending_lease_until_unix_seconds: optional_nonnegative(row.get(24)?),
        targets: Vec::new(),
        created_unix_seconds: nonnegative(row.get(25)?),
        updated_unix_seconds: nonnegative(row.get(26)?),
    })
}
...
fn dns_switch_reason(
    ...
    if current_peer_id == candidate_peer_id {
        return Ok("target_value_changed");
    }
    let current = connection
        .query_row(
