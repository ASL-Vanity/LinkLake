...
void main() {
  ...
  testWidgets('saving keeps DNS-01 and excludes credential readiness fields', (
    tester,
  ) async {
    ...
    await tester.pumpWidget(
      MaterialApp(
        home: Scaffold(
          body: AcmeSettingsPage(api: api, chinese: false, editable: true),
        ),
      ),
    );
  ...
  testWidgets('auditor can inspect readiness but cannot save', (tester) async {
    ...
    await tester.pumpWidget(
      MaterialApp(
        home: Scaffold(
          body: AcmeSettingsPage(api: api, chinese: false, editable: false),
        ),
      ),
    );
    await tester.pumpAndSettle();
  ...
  testWidgets('legacy missing key status is not reported ready', (
    tester,
  ) async {
    final api = configuredApi();
    api.objectResponses['/api/v1/acme/config']!.remove(
      'material_key_configured',
    );
    await tester.pumpWidget(
      MaterialApp(
        home: Scaffold(
