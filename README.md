...
`--data-dir` 必须由安装器或管理员提前安全创建，不能是符号链接、重解析点或由备份命令临时创建的任意目录。Linux 建议归服务账户所有并设为 `0700`；Windows 应只授权 SYSTEM、Administrators、LocalService 和该目录所有者。备份与恢复会拒绝不存在或不安全的数据目录，而不会替管理员猜测权限边界。
