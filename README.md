...
- Secret 提供端必须使用支持目标绑定的客户端；旧客户端会收到明确的升级错误并被拒绝注册，避免在健康状态未知时绕过服务端目标选择。
...
`v1.0.0` 已完成核心协议与首个跨平台管理版本。项目不再单独发布 `v1.0.1`；管理界面完善、安全远程更新、协议补全、业务健康与 Fleet、PostgreSQL 高可用、平台安装器和依赖现代化统一纳入 `v1.1.0`。全部功能冻结后才集中执行完整测试、灰度和正式发布。范围、验收标准和持续支持边界见 [`ROADMAP.md`](ROADMAP.md)。
